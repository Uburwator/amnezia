...
This document covers all issues encountered during development and testing,
with solutions.
...
iptables -A FORWARD -i awg0 -o eth0 -j ACCEPT \
  2>/dev/null || true
iptables -A FORWARD -i awg0 -o eth1 -j ACCEPT \
  2>/dev/null || true
iptables -A FORWARD -o awg0 -m state \
  --state RELATED,ESTABLISHED -j ACCEPT 2>/dev/null || true
iptables -t nat -A POSTROUTING -s ${VPN_SUBNET} \
  -o eth0 -j MASQUERADE 2>/dev/null || true
iptables -t nat -A POSTROUTING -s ${VPN_SUBNET} \
  -o eth1 -j MASQUERADE 2>/dev/null || true
...
If default route points to a network without internet (like amnezia-dns-net
`172.29.172.0/24`), the NAT rules to BOTH eth0 and eth1 in start.sh should
fix it.

#### Cause 3: Multiple Docker networks interfering

The container might have multiple network interfaces. The solution is to NAT
to ALL of them (eth0, eth1) which the updated start.sh does.
...
If they don't match, regenerate the client with the latest add-client
script.
...
Ensure start.sh has proper FORWARD and MASQUERADE rules (see "Container
missing NAT rules" above).
...
The add-client script had a bug where it tried to `source` a params file
with wrong format. Latest version reads directly from server config.
...
**Cause:**
Wrong QR code format. iOS requires specific Base64 encoding **without** the
`vpn://` prefix.

**Solution:**

Use the updated add-client.sh script (v1.1+) which generates **Format 6**
QR codes.
...
Hex strings in `<b 0xHEX>` tags must have even number of characters (2 hex
digits = 1 byte).
...
The latest script has correct I5 with even-length hex strings. Don't
manually edit I1-I5 values.
...
**This is OK!** The message says "Falling back to slow userspace
implementation" which works fine. The Docker image includes the userspace
implementation.
...
#   ← Points to DNS network (no internet)
...
Container connected to multiple Docker networks (`amnezia-dns-net` and
default `bridge`). Default route might point to wrong network.

**Solution:**

The start.sh script NATs to BOTH eth0 and eth1, so traffic can exit through
either interface:
...
```bash
docker exec container awg syncconf awg0 \
  <(docker exec container awg-quick strip config)
```
...
docker exec container sh -c \
  "awg-quick strip /path/config > /tmp/stripped.conf"
...
tcpdump -i eth0 -n udp port 51821
...
- **Allow**: UDP port 51821 (or your configured port)
- **Direction**: Inbound
...
tcpdump -i eth0 -n 'udp port 51821 and host [CLIENT_IP]'
...
See "Client Connects But No Internet" → "Cause 1: Container missing
NAT rules"
...
iptables -I DOCKER-USER -d 10.66.66.0/24 -m conntrack \
...
iOS AmneziaVPN app v4.8.14 has issues with QR code compression format.
...
- Safari → `http://10.66.66.1`
  (should timeout - server doesn't run HTTP)
...
1. **Docker multi-network routing**: Container with multiple network
   interfaces needs NAT to ALL of them
2. **Process substitution limitation**: `<(...)` doesn't cross docker exec
   boundary
3. **iOS QR code issues**: Compression format incompatible, file import
   works
4. **Parameter format**: `source` command fails with WireGuard format
   (`Key = value` with spaces)
5. **I1-I5 hex encoding**: Must have even number of hex digits
6. **Container network isolation**: iptables rules needed INSIDE container,
   not just on host
